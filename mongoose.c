...
int mg_http_parse_header(struct mg_str *hdr, const char *var_name, char *buf,
   ...
   const char *p, *end = hdr ? hdr->ptr + hdr->len : NULL, *s = NULL;
   
   if (buf != NULL && buf_size > 0) buf[0] = '\0';
   if (hdr == NULL) return 0;
   
   /* Find where variable starts */
   for (s = hdr->ptr; s != NULL && s + n < end; s++) {
      if ((s == hdr->ptr || s[-1] == ch || s[-1] == ch1) && s[n] == '=' &&
          !memcmp(s, var_name, n))
         break;
   }
...
// size_t ret =
      //     mg_parse_multipart(hm->body.ptr, hm->body.len, cmd, sizeof(cmd),
      //                        filename, sizeof(filename), &data, &data_len);

      // printf("got value : var_name %s file name %s data %s datalen is %d\r\n",
      //        cmd, filename, data, data_len);
      // strncpy(filename, data, data_len);
      // filename[data_len] = '\0';
      // if (mg_globmatch("ssid", strlen("ssid"), cmd, strlen(cmd))) {
      //   printf("got user is %s\r\n", filename);
      // }
      // memset(cmd, 0, sizeof(cmd));
      // memset(filename, 0, sizeof(filename));
      // ret = mg_parse_multipart(data + data_len + 2, strlen(data + data_len + 2),
      //                          cmd, sizeof(cmd), filename, sizeof(filename),
      //                          &data1, &data_len);
      // // ret = mg_parse_multipart(hm->body.ptr + (hm->body.len - ret), ret, cmd,
      // // sizeof(cmd), filename, sizeof(filename), &data1, &data_len);

      // printf("got value : var_name %s file name %s data %s datalen is %d\r\n",
      //        cmd, filename, data1, data_len);
      // strncpy(filename, data1, data_len);
      // filename[data_len] = '\0';
      // if (mg_globmatch("pwd", strlen("pwd"), cmd, strlen(cmd))) {
      //   printf("got pwd is %s\r\n", filename);
      // }
size_t mg_parse_multipart(const char *buf, size_t buf_len, char *var_name,
   ...
   if ((hl = mg_http_get_request_len(buf, buf_len)) <= 0) {printf("request len failed\r\n"); return 0;}
  
   //if (buf[0] != '-' || buf[1] != '-' || buf[2] == '\n') {printf("check condition failed\r\n"); return 0;}
   
   /* Get boundary length */
   const char* posi = strstr(buf, cd);
   bl = (posi - buf);
...
// int mg_http_get_request_len(const unsigned char *s, size_t buf_len) {
